...
void refresh_input_state(s_input_state *const input_state, GLFWwindow *const glfw_window)
{
    ...
    for (int i = 0; i < k_key_code_cnt; ++i)
    {
        ...
        {
            input_state->keys_down_bits |= static_cast<u_keys_down_bits>(1) << i;
        }
    }

    // Refresh the mouse button state.
    for (int i = 0; i < k_mouse_button_code_cnt; ++i)
    {
        if (glfwGetMouseButton(glfw_window, i) == GLFW_PRESS)
        {
            input_state->mouse_buttons_down_bits |= static_cast<u_mouse_buttons_down_bits>(1) << i;
        }
    }
    ...
    for (int i = 0; i <= GLFW_JOYSTICK_LAST; i++)
    {
        if (glfwJoystickPresent(i) && glfwJoystickIsGamepad(i))
        {
            ...
            if (glfwGetGamepadState(i - 1, &glfw_gamepad_state))
            {
                for (int j = 0; j < k_gamepad_button_code_cnt; j++)
                {
                    if (glfw_gamepad_state.buttons[j] == GLFW_PRESS)
                    {
                        input_state->gamepad_buttons_down_bits |= static_cast<u_gamepad_buttons_down_bits>(1) << j;
                    }
                }

                for (int j = 0; j < k_gamepad_axis_code_cnt; j++)
                {
                    input_state->gamepad_axis_values[j] = glfw_gamepad_state.axes[j];
                }
            }
            else
            {
                cc::log_error("Failed to retrieve the state of gamepad with GLFW joystick number %d.", i);
                break;
            }

            input_state->gamepad_glfw_joystick_num = i + 1;
